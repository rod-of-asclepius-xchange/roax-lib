...
1,122 is `check_corpus.mjs` run here without the reference checkout, which passes 1,114 assertions and reports class 10 NOT RUN, plus the 8 assertions those four vectors carry once the checkout is supplied.
76 is counted from `validate_schemas.mjs`'s structure - the corpus file, the four corpus type maps, the 54 envelope fixtures and its 17 conditional probes - rather than read off a run, because that tool needs an Ajv installed outside this tree.
