...
Specification, schemas, the conformance corpus, and **one library: the TypeScript implementation
under `src/`.**

The specifications came first so the design could be reviewed before five language implementations
existed to be re-litigated. That ordering held, and the TypeScript build is the first of the five
the project owner has now ruled for.

**Do not add a Rust crate, Go module, Swift package or Kotlin library without an explicit
instruction to do so.** The ruling was for TypeScript first, not for all five at once.
...
superseded **for TypeScript only**: adding a `Cargo.toml`, a `go.mod`, a `Package.swift` or a
Gradle build is still the thing not to do without an instruction. The package is zero-dependency at
