...
**The filename inside `ROAX_REFERENCE_RECORDS` is `<authority>.<profile>.json`**, and class 10 now
needs TWO of them: `sg.gov.moh.recovery-healthcert.json` and
`sg.gov.moh.vaccination-healthcert.json`, since the two vaccination bindings were ruled on
2026-07-30 and its sample commits.
Each is named independently, so a directory holding one leaves the other profile's two vectors NOT
RUN and the class short rather than failing.
